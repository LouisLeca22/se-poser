...
export const toggleFavoriteAction = async (prevState: { propertyId: string, favoriteId: string | null, pathname: string }) => {
    ...
    try {
        ...
        return { message: favoriteId ? "Retiré des favoris" : "ajouté aux favoris" }
...
export const createReviewAction = async (prev: any, formData: FormData) => {
    ...
    try {
        ...
        return { message: 'Votre avis a été ajouté' };
    } catch (error) {
        return renderError(error)
    }
};
...
export const createBookingAction = async (prevState: { propertyId: string, checkIn: Date, checkOut: Date }) => {
    ...
    if (!property) {
        return { message: "Cet hébérgement n'existe pas" }
    }
...
export const fetchRentals = async () => {
    ...
    const rentalsWithBookingsSums = await Promise.all(
        rentals.map(async (rental) => {
            ...
            return {
                ...rental, totalNightsSum: totalNightsSum._sum.totalNights, orderTotalSum: orderTotalSum._sum.orderTotal
            }
        })
...
export const deleteRentalAction = async (prevState: { propertyId: string }) => {
    ...
    try {
        ...
        return { message: "Location supprimée" }
    } catch (error) {
        return renderError(error)
    }
}
